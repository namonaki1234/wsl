...
double precision,dimension(0:IM,0:JM)::xixS,xiyS,etaxS,etayS,JJS,uS,vS,TS,&
                                       uxiS,vxiS,TxiS,uetaS,vetaS,TetaS !τij用偏微分（スタガード格子）
double precision::txxS,txyS,tyyS !τxx,τxy,τyy（スタガード格子）
double precision,dimension(1:4,0:IM,0:JM)::RvS=0.0d0,SvS=0.0d0,RvSS=0.0d0,SvSS=0.0d0 !粘性項ベクトルR,S（スタガード格子）
...
do while(dq>EPS.and.n<nmax)
 ...
 do nn=1,4
  ...
  do j=1,JM-1 !粘性項ベクトル
   do i=0,IM-1
    txxS=2.0d0*muS(i,j)*(2.0d0*(uxiS(i,j)*xixS(i,j)+uetaS(i,j)*etaxS(i,j))-(vxiS(i,j)*xiyS(i,j)+vetaS(i,j)*etayS(i,j)))/3.0d0
    txyS=muS(i,j)*((uxiS(i,j)*xiyS(i,j)+uetaS(i,j)*etayS(i,j))+(vxiS(i,j)*xixS(i,j)+vetaS(i,j)*etaxS(i,j)))
    tyyS=2.0d0*muS(i,j)*(2.0d0*(vxiS(i,j)*xiyS(i,j)+vetaS(i,j)*etayS(i,j))-(uxiS(i,j)*xixS(i,j)+uetaS(i,j)*etaxS(i,j)))/3.0d0
    RvS(2,i,j)=txxS
    RvS(3,i,j)=txyS
    RvS(4,i,j)=txxS*uS(i,j)+txyS*vS(i,j)+muS(i,j)*g*R*(TxiS(i,j)*xixS(i,j)+TetaS(i,j)*etaxS(i,j))/(Pr*(g-1.0d0))
    SvS(2,i,j)=txyS
    SvS(3,i,j)=tyyS
    SvS(4,i,j)=txyS*uS(i,j)+tyyS*vS(i,j)+muS(i,j)*g*R*(TxiS(i,j)*xiyS(i,j)+TetaS(i,j)*etayS(i,j))/(Pr*(g-1.0d0))
    do k=1,4 !流束を一般座標系に変換
     RvSS(k,i,j)=(xixS(i,j)*RvS(k,i,j)+xiyS(i,j)*SvS(k,i,j))/JJS(i,j)
    end do
   end do
  end do
  ...
  do j=0,JM-1 !粘性項ベクトル
   do i=1,IM-1
    ...
    SvS(4,i,j)=txyS*uS(i,j)+tyyS*vS(i,j)+muS(i,j)*g*R*(TxiS(i,j)*xiyS(i,j)+TetaS(i,j)*etayS(i,j))/(Pr*(g-1.0d0))
    do k=1,4 !各流束を一般座標系に変換
     SvSS(k,i,j)=(etaxS(i,j)*RvS(k,i,j)+etayS(i,j)*SvS(k,i,j))/JJS(i,j)
    end do
   end do
  end do
   ...
   do i=1,IM-1
    do k=1,4
     QQ(k,i,j,nn)=QQ(k,i,j,0)-dt*(EE(k,i,j)-EE(k,i-1,j)+FF(k,i,j)-FF(k,i,j-1)&
                  -(RvSS(k,i,j)-RvSS(k,i-1,j)+SvSS(k,i,j)-SvSS(k,i,j-1)))/dble(5-nn)
     Q(k,i,j)=QQ(k,i,j,nn)
    end do
   end do
  end do
...
!.datファイル書き出し***************************************
open(1,file='comp7.dat',status='replace')
do j=0,JM
 do i=0,IM
...
!.fldファイル作成*******************************************
open(2,file='comp7.fld',status='replace')
...
write(2,'(A)') 'variable 1 file=comp7.dat filetype=ascii skip=0 offset=2 stride=6'
write(2,'(A)') 'variable 2 file=comp7.dat filetype=ascii skip=0 offset=3 stride=6'
write(2,'(A)') 'variable 3 file=comp7.dat filetype=ascii skip=0 offset=4 stride=6'
write(2,'(A)') 'variable 4 file=comp7.dat filetype=ascii skip=0 offset=5 stride=6'
write(2,'(A)') 'coord 1 file=comp7.dat filetype=ascii skip=0 offset=0 stride=6'
write(2,'(A)') 'coord 2 file=comp7.dat filetype=ascii skip=0 offset=1 stride=6'
close(2)
