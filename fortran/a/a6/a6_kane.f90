...
jj
double precision,dimension(1:4,0:im,0:jm)::q,ee,ff,rv,sv
...
open(17,file='kadai6.dat',status='replace')
   do j=0,jm
      do i=0,im
...
open(18,file='kadai6.fld',status='replace')
...
write(18,'(a)')'variable 1 file=kadai6.dat filetype=ascii skip=0 offset=2 stride=6'
write(18,'(a)')'variable 2 file=kadai6.dat filetype=ascii skip=0 offset=3 stride=6'
write(18,'(a)')'variable 3 file=kadai6.dat filetype=ascii skip=0 offset=4 stride=6'
write(18,'(a)')'variable 4 file=kadai6.dat filetype=ascii skip=0 offset=5 stride=6'
write(18,'(a)')'coord 1 file=kadai6.dat filetype=ascii skip=0 offset=0 stride=6'
write(18,'(a)')'coord 2 file=kadai6.dat filetype=ascii skip=0 offset=1 stride=6'
close(18)


contains

double precision function sland(t)   !サザーランドの式によるμの導出
double precision t
...
do j=0,jm
   do i=1,im-1
      ...
      txi(i,j)=((t(i+1,j)-t(i-1,j))/2.0d0)
   end do
end do
do i=0,im
   ueta(i,0)=(-3.0d0*u(i,0)+4.0d0*u(i,1)-u(i,2))/(2.0d0*ii)
   ueta(i,jm)=(3.0d0*u(i,jm)-4.0d0*u(i,jm-1)+u(i,jm-2))/(2.0d0*ii)
   veta(i,0)=(-3.0d0*v(i,0)+4.0d0*v(i,1)-v(i,2))/(2.0d0*ii)
   veta(i,jm)=(3.0d0*v(i,jm)-4.0d0*v(i,jm-1)+v(i,jm-2))/(2.0d0*ii)
   teta(i,0)=(-3.0d0*t(i,0)+4.0d0*t(i,1)-t(i,2))/(2.0d0*ii)
   teta(i,jm)=(3.0d0*t(i,jm)-4.0d0*t(i,jm-1)+t(i,jm-2))/(2.0d0*ii)
end do
do j=0,jm
   uxi(0,j)=(-3.0d0*u(0,j)+4.0d0*u(1,j)-u(2,j))/(2.0d0*ii)
   uxi(im,j)=(3.0d0*u(im,j)-4.0d0*u(im-1,j)+u(im-2,j))/(2.0d0*ii)
   vxi(0,j)=(-3.0d0*v(0,j)+4.0d0*v(1,j)-v(2,j))/(2.0d0*ii)
   vxi(im,j)=(3.0d0*v(im,j)-4.0d0*v(im-1,j)+v(im-2,j))/(2.0d0*ii)
   txi(0,j)=(-3.0d0*t(0,j)+4.0d0*t(1,j)-t(2,j))/(2.0d0*ii)
   txi(im,j)=(3.0d0*t(im,j)-4.0d0*t(im-1,j)+t(im-2,j))/(2.0d0*ii)
end do

do j=0,jm
   do i=0,im
mu=sland(t(i,j))
txx=2.0d0*mu/3.0d0*(2.0d0*(uxi(i,j)*xix(i,j)+ueta(i,j)*etax(i,j))-&
                   (vxi(i,j)*xiy(i,j)+veta(i,j)*etay(i,j)))
tyy=2.0d0*mu/3.0d0*(2.0d0*(vxi(i,j)*xiy(i,j)+veta(i,j)*etay(i,j))-&
                   (uxi(i,j)*xix(i,j)+ueta(i,j)*etax(i,j)))
txy=mu*((uxi(i,j)*xiy(i,j)+ueta(i,j)*etay(i,j))+&
        (vxi(i,j)*xix(i,j)+veta(i,j)*etax(i,j)))
tyx=txy
r4=txx*u(i,j)+txy*v(i,j)+(mu/(pr*(g-1.0d0)))*g*r*&
        (txi(i,j)*xix(i,j)+teta(i,j)*etax(i,j))
s4=tyx*u(i,j)+tyy*v(i,j)+(mu/(pr*(g-1.0d0)))*g*r*&
        (txi(i,j)*xiy(i,j)+teta(i,j)*etay(i,j))
rv(1,i,j)=0.0d0
rv(2,i,j)=(xix(i,j)*txx+xiy(i,j)*txy)/jj(i,j)
rv(3,i,j)=(xix(i,j)*tyx+xiy(i,j)*tyy)/jj(i,j)
rv(4,i,j)=(xix(i,j)*r4+xiy(i,j)*s4)/jj(i,j)
sv(1,i,j)=0.0d0
sv(2,i,j)=(etax(i,j)*txx+etay(i,j)*txy)/jj(i,j)
sv(3,i,j)=(etax(i,j)*tyx+etay(i,j)*tyy)/jj(i,j)
sv(4,i,j)=(etax(i,j)*r4+etay(i,j)*s4)/jj(i,j)
   end do
end do
...
QQ(k,i,j,mm)=QQ(k,i,j,0)-(1.0d0/dble(5-mm))*dt*(-EE(k,i-1,j)+EE(k,i,j)-&
             FF(k,i,j-1)+FF(k,i,j)+((rv(k,i-1,j)-rv(k,i+1,j))/2.0d0)+&
             ((sv(k,i,j-1)-sv(k,i,j+1))/2.0d0))
q(k,i,j)=qq(k,i,j,mm)
      end do
   end do
end do
