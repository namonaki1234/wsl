...
double precision,parameter :: R = 287.1d0,g = 1.4d0,dt = 1.0d-8,T0 = 293.0d0,myu0 = 1.0d-3,Suther = 110.0d0,Pr = 0.72d0
integer :: i,j,n,k,rk
...
!グラフ作成
open (1,file='asshuku_6.dat',status='replace')
do j = 0,JM
  do i = 0,IM
...
double precision function Sutherland(T)
  double precision T
  Sutherland = myu0*((T/T0)**1.5)*(T0+Suther)/(T+Suther)
end function

!------粘性項------
subroutine viscosity
  myu = Sutherland(T(i,j))

  do j = 0,JM
    do i = 1,IM-1
      ue(i,j) = (u(i+1,j)-u(i-1,j))/2.0d0
      ve(i,j) = (v(i+1,j)-v(i-1,j))/2.0d0
      Te(i,j) = (T(i+1,j)-T(i-1,j))/2.0d0
    end do
  end do
  do j = 1,JM-1
    do i = 0,IM
      ...
      Th(i,j) = (T(i,j+1)-T(i,j-1))/2.0d0
    end do
  end do
  do j = 0,JM
    ue(0,j) = (-3.0d0*u(0,j)+4.0d0*u(1,j)-u(2,j))/2.0d0
    ue(IM,j) = (3.0d0*u(IM,j)-4.0d0*u(IM-1,j)+u(IM-2,j))/2.0d0
    ve(0,j) = (-3.0d0*v(0,j)+4.0d0*v(1,j)-v(2,j))/2.0d0
    ve(IM,j) = (3.0d0*v(IM,j)-4.0d0*v(IM-1,j)+v(IM-2,j))/2.0d0
    Te(0,j) = (-3.0d0*T(0,j)+4.0d0*T(1,j)-T(2,j))/2.0d0
    Te(IM,j) = (3.0d0*T(IM,j)-4.0d0*T(IM-1,j)+T(IM-2,j))/2.0d0
  end do

  do i = 0,IM
    uh(i,0) = (-3.0d0*u(i,0)+4.0d0*u(i,1)-u(i,2))/2.0d0
    uh(i,JM) = (3.0d0*u(i,JM)-4.0d0*u(i,JM-1)+u(i,JM-2))/2.0d0
    vh(i,0) = (-3.0d0*v(i,0)+4.0d0*v(i,1)-v(i,2))/2.0d0
    vh(i,JM) = (3.0d0*v(i,JM)-4.0d0*v(i,JM-1)+v(i,JM-2))/2.0d0
    Th(i,0) = (-3.0d0*T(i,0)+4.0d0*T(i,1)-T(i,2))/2.0d0
    Th(i,JM) = (3.0d0*T(i,JM)-4.0d0*T(i,JM-1)+T(i,JM-2))/2.0d0
  end do

  do j = 0,JM
    do i = 0,IM
      txx = 2.0d0/3.0d0*myu*(2.0d0*(ue(i,j)*xix(i,j)+uh(i,j)*ex(i,j))-(ve(i,j)*xiy(i,j)+vh(i,j)*ey(i,j)))
      tyy = 2.0d0/3.0d0*myu*(2.0d0*(ve(i,j)*xiy(i,j)+vh(i,j)*ey(i,j))-(ue(i,j)*xix(i,j)+uh(i,j)*ex(i,j)))
      txy = myu*((ue(i,j)*xiy(i,j)+uh(i,j)*ey(i,j))+(ve(i,j)*xix(i,j)+vh(i,j)*ex(i,j)))
      tyx = txy
      R4 = txx*u(i,j)+txy*v(i,j)+myu/(Pr*(g-1.0d0))*g*r*(Te(i,j)*xix(i,j)+Th(i,j)*ex(i,j))
      S4 = tyx*u(i,j)+tyy*v(i,j)+myu/(Pr*(g-1.0d0))*g*r*(Te(i,j)*xiy(i,j)+Th(i,j)*ey(i,j))
      RRR(1,i,j) = 0.0d0
      RRR(2,i,j) = (xix(i,j)*txx+xiy(i,j)*txy)/Jac(i,j)
      RRR(3,i,j) = (xix(i,j)*tyx+xiy(i,j)*tyy)/Jac(i,j)
      RRR(4,i,j) = (xix(i,j)*R4+xiy(i,j)*S4)/Jac(i,j)
      SSS(1,i,j) = 0.0d0
      SSS(2,i,j) = (ex(i,j)*txx+ey(i,j)*txy)/Jac(i,j)
      SSS(3,i,j) = (ex(i,j)*tyx+ey(i,j)*tyy)/Jac(i,j)
      SSS(4,i,j) = (ex(i,j)*R4+ey(i,j)*S4)/Jac(i,j)
    end do
  end do
