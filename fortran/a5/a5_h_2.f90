...
do j = 0,JM
  do i = 0,IM
    if (y(i,j)<=y(i,JM-2) .and. x(i,j)<=dsqrt(3.d0)*(y(i,JM-2)-y(i,j))) cycle
    T(i,j) = T(i,j)*(2.d0*gam*(M(i,j)**2)*(dsin(beta)**2)-(gam-1.d0)) &
             *((gam-1.0d0)*(M(i,j)**2)*(dsin(beta)**2)+2.d0) &
             /(((gam+1.0d0)**2)*(M(i,j)**2)*(dsin(beta)**2))

    p(i,j) = p(i,j)*(2.d0*gam*(M(i,j)**2)*(dsin(beta)**2)-(gam-1.d0)) &
             /(gam+1.0d0)
