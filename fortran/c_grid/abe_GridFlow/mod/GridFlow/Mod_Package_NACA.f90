...
!********					      2016.10.26     UPDATED BY SHO     URANAI  ********
!*******************************************************************************************************
!*******************************************************************************************************
module Package_NACA
 ! 変数宣言 ********************************************************************************************
 ...
 type Type_Flow
  ...
  integer, pointer :: HeatFlag(:)					! 加熱位置
  integer, pointer :: ic(:, :), ji(:)					! 着氷セル	!ICM用に追加
 ...
 integer, parameter :: swi_ICM = 1					! ICMの有無（0：なし，1：あり）
 integer :: swi_subgrid	= 2						! サブグリッドの種類(1:H,2:C,3:O)
 ...
 integer :: DragNum							! 液滴に働く抗力係数のモデル選択
 real    :: span, chord, AOA						! スパン長，コード長，迎角
 real    :: VelExp, TsExp, PsExp, ThExp, HeatRange			! 実験条件
 ...
 &            FlwIniDir * 50, FlwIniClDir * 50,FlwCalInDir * 50, FlwCalOutDir * 50, FlwViewDir * 50, IceViewDir * 50, &
...
subroutine Input_CalSetting(strname)
 ...
 open(1, file = trim(strname), form = 'formatted', status = 'old')
  ...
  read(1, '(e16.8e3,4(x,e16.8e3))') VelExp, PsExp, TsExp, ThExp, HeatRange
...
subroutine Output_CalSetting(strname)
 ...
 open(1, file = trim(strname), form = 'formatted', status = 'replace')
  ...
  write(1, '(e16.8e3,4(x,e16.8e3))') VelExp, PsExp, TsExp, ThExp,HeatRange
...
subroutine Input_ExpCondition(strname)
 ...
 open(1, file = trim(strname), form = 'formatted', status = 'old')
  ...
  read(1, *) ThExp							! [℃]
  read(1, *) HeatRange							! [%]
  read(1, *) VelExp							! [m/s] 流速
  read(1, *) PsExp							! [kPa]
  read(1, *) MVD							! [μm] 平均液滴直径
  read(1, *) LWC							! [g/m^3] 水分含有量
  ...
  read(1, *) muD							! [N・s/m^2]
 close(1)
 TsExp      = TsExp + IceTs ;  ThExp      = ThExp + IceTs
 PsExp      = PsExp * 1.0e+3
 MVD        = MVD * 1.0e-6
 LWC        = LWC * 1.0e-3
