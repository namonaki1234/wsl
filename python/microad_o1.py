...
for _ in range(n - 1):
    ...
    dsu.merge(p - 1, c - 1)  # 0-indexedに変換

# クエリの数
q = int(input())
# クエリを処理
for _ in range(q):
    u = int(input()) - 1  # 0-indexedに変換
    path = []
    current = u
    ...
    if current == 1:
      ...
      continue
    # 親を辿っていく
    while current != 0 and current != 1:  # 根ノードは0
        path.append(a[current])
        current = dsu.leader(current)  # 親ノードを取得

        for i in range()


    path.reverse()  # 根からの順番にする
    print('/'.join(path))  # `/` 区切りで出力
