...
open(17,file='kadai7.dat',status='replace')
   do j=0,jm
      do i=0,im
...
open(18,file='kadai7.fld',status='replace')
...
write(18,'(a)')'variable 1 file=kadai7.dat filetype=ascii skip=0 offset=2 stride=6'
write(18,'(a)')'variable 2 file=kadai7.dat filetype=ascii skip=0 offset=3 stride=6'
write(18,'(a)')'variable 3 file=kadai7.dat filetype=ascii skip=0 offset=4 stride=6'
write(18,'(a)')'variable 4 file=kadai7.dat filetype=ascii skip=0 offset=5 stride=6'
write(18,'(a)')'coord 1 file=kadai7.dat filetype=ascii skip=0 offset=0 stride=6'
write(18,'(a)')'coord 2 file=kadai7.dat filetype=ascii skip=0 offset=1 stride=6'
close(18)
...
subroutine visxi   !ξ方向粘性項
double precision,dimension(0:im-1,0:jm-1)::xixi,xiyi,etaxi,etayi,jji,ui,vi,ti,&
uxii,vxii,txii,uetai,vetai,tetai
...
do j=0,jm-1
   do i=0,im-1
mu=sland(ti(i,j))
txx=2.0d0*mu/3.0d0*(2.0d0*(uxii(i,j)*xixi(i,j)+uetai(i,j)*etaxi(i,j))-&
                   (vxii(i,j)*xiyi(i,j)+vetai(i,j)*etayi(i,j)))
tyy=2.0d0*mu/3.0d0*(2.0d0*(vxii(i,j)*xiyi(i,j)+vetai(i,j)*etayi(i,j))-&
                   (uxii(i,j)*xixi(i,j)+uetai(i,j)*etaxi(i,j)))
txy=mu*((uxii(i,j)*xiyi(i,j)+uetai(i,j)*etayi(i,j))+&
        (vxii(i,j)*xixi(i,j)+vetai(i,j)*etaxi(i,j)))
tyx=txy
r4=txx*ui(i,j)+txy*vi(i,j)+(mu/(pr*(g-1.0d0)))*g*r*&
        (txii(i,j)*xixi(i,j)+tetai(i,j)*etaxi(i,j))
s4=tyx*ui(i,j)+tyy*vi(i,j)+(mu/(pr*(g-1.0d0)))*g*r*&
...
do j=0,jm-1
   do i=0,im-1
mu=sland(te(i,j))
txx=2.0d0*mu/3.0d0*(2.0d0*(uxie(i,j)*xixe(i,j)+uetae(i,j)*etaxe(i,j))-&
                   (vxie(i,j)*xiye(i,j)+vetae(i,j)*etaye(i,j)))
tyy=2.0d0*mu/3.0d0*(2.0d0*(vxie(i,j)*xiye(i,j)+vetae(i,j)*etaye(i,j))-&
                   (uxie(i,j)*xixe(i,j)+uetae(i,j)*etaxe(i,j)))
txy=mu*((uxie(i,j)*xiye(i,j)+uetae(i,j)*etaye(i,j))+&
        (vxie(i,j)*xixe(i,j)+vetae(i,j)*etaxe(i,j)))
tyx=txy
r4=txx*ue(i,j)+txy*ve(i,j)+(mu/(pr*(g-1.0d0)))*g*r*&
        (txie(i,j)*xixe(i,j)+tetae(i,j)*etaxe(i,j))
s4=tyx*ue(i,j)+tyy*ve(i,j)+(mu/(pr*(g-1.0d0)))*g*r*&
...
QQ(k,i,j,mm)=QQ(k,i,j,0)-(1.0d0/dble(5-mm))*dt*(-EE(k,i-1,j)+EE(k,i,j)-&
             FF(k,i,j-1)+FF(k,i,j)+(rv(k,i-1,j)-rv(k,i,j))+&
             (sv(k,i,j-1)-sv(k,i,j)))
q(k,i,j)=qq(k,i,j,mm)
      end do
   end do
