...
!***********************************************************************
!**** 乱流モデル : RANS, EVM, low Reynolds number k-e Model (1992)  ****
!****                         Abe-Nagano-Kondo                      ****
!**** 計算対象 : 単相, 三次元, 圧縮性                               ****
!**** 計算スキーム : 二次精度中心差分(スタガード格子上の点を使用)   ****
!***********************************************************************
SUBROUTINE Turbulence3DEvmANKlRe( &
&            PELIM, RG, GAMMA, PR, PRT, &
...
&          )
  ...
  REAL, PARAMETER  :: ZERO = 1.0E-20
  REAL, PARAMETER  :: D = 0.0, E = 0.0
  REAL, PARAMETER  :: F1 = 1.0
  REAL             :: FMU
  REAL, DIMENSION(IS: IE, JS: JE, KS: KE) :: F2
  DOUBLE PRECISION :: RT, ETA, YST !乱流レイノルズ数，コルモゴロフのマイクロスケール，Y*
  ! 引数変数 +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  REAL,    INTENT(IN)  :: PELIM
  REAL,    INTENT(IN)  :: RG, GAMMA, PR, PRT
  INTEGER, INTENT(IN)  :: IS, IE, JS, JE, KS, KE
...
SUBROUTINE NonDiffTerm
  ...
  IF(QH(I,J,K,1) .GT. 0.0 .AND. QH(I,J,K,7) .GT. ZERO) THEN
   !各種追加関数
   RT = DBLE(QH(I,J,K,1) * AJA(I,J,K) * AK(I,J,K)**2.0) / DBLE(AMU(I,J,K) * EPS(I,J,K))
!QH(I,J,K,6)**2.0 / (AMU(I,J,K) * QH(I,J,K,7)) * AJA(I,J,K)
   ETA = (DBLE(AMU(I,J,K) / (QH(I,J,K,1) * AJA(I,J,K)))**3.0 / DBLE(EPS(I,J,K)))**0.25
!(AMU(I,J,K) **3.0 / (QH(I,J,K,1)**2.0 * QH(I,J,K,7) * AJA(I,J,K)**3.0))**0.25
   IF(I .GT. I1 .AND. I .LT. I3) THEN
    YST = DSQRT(DBLE(X(I,J,K) - X(I,JS,K))**2.0 +  &
    &           DBLE(Y(I,J,K) - Y(I,JS,K))**2.0 +  &
    &           DBLE(Z(I,J,K) - Z(I,JS,K))**2.0) / ETA
   ELSE
    YST = DSQRT(DBLE(X(I,J,K) - X(I1,JS,K))**2.0 +  &
    &           DBLE(Y(I,J,K) - Y(I1,JS,K))**2.0 +  &
    &           DBLE(Z(I,J,K) - Z(I1,JS,K))**2.0) /ETA
   ENDIF
   FMU = REAL((1.0 - DEXP(-YST / 14.0))**2.0 * (1.0 + 5.0 / RT**(3.0/4.0) * DEXP(-(RT / 200.0)**2.0)))
   F2(I,J,K) = REAL((1.0 - DEXP(-YST / 3.1))**2.0 * (1.0 - 0.3 * DEXP(-(RT / 6.5)**2.0)))
!write(*,*) QH(I,J,K,7),RT,ETA, YST ,fmu,f2
   !渦粘性係数
   AMUT(I,J,K) = CMU * FMU * (QH(I,J,K,6) * AJA(I,J,K))**2 / (QH(I,J,K,7) * AJA(I,J,K))
!    AMUT(I,J,K) = CMU * QH(I,J,K,6)**2 / QH(I,J,K,7) * AJA(I,J,K)

!if(ie .eq. 800) then
!if((I .le. I1 .or. I .ge. I3) .and. j .eq. js) write(*,*) i,real(AMUT(I,JS,K))
!!if((I .le. I1 .or. I .ge. I3) .and. j .eq. je) write(*,*) i,j,real(AMUT(I,JE,K)) !,real(eta),real(yst)
!end if

  ENDIF
  IF(AMUT(I,J,K) .ne. AMUT(I,J,K)) THEN
   WRITE(*,*) 'ERROR: MU_T IS DIVERGING'
   WRITE(*,*)  QH(I,J,K,7),RT,ETA, YST ,FMU,F2(I,J,K)
   STOP
  ENDIF
  ENDDO
  ENDDO
  ...
  & ) THEN
    ...
    &       QH(I,J,K,7) / MAX(ZERO, QH(I,J,K,6)) &
    &     )
    DQP(I,J,K,6) = PRO/AJA(I,J,K) - QH(I,J,K,7)+D
    DQP(I,J,K,7) = (CE1*F1*PRO/AJA(I,J,K)-CE2*F2(I,J,K)*QH(I,J,K,7))*EPSPK+E
  ENDIF
  ENDDO
  ENDDO
  ENDDO
...
END SUBROUTINE VISZE
! 定義終了 *************************************************************
END SUBROUTINE Turbulence3DEvmANKlRe
...
&          )
  ...
  CALL DiffTerm
  ! メモリ解放 =========================================================
  DEALLOCATE(RH,SH,UXXI,UYXI,VXXI,VYXI,UXET,UYET,VXET,VYET)
  ! 処理終了 ***********************************************************
  RETURN
! 内部手続き ***********************************************************
CONTAINS
...
&          )
  ...
  CALL DiffTerm
  ! メモリ解放 =========================================================
  DEALLOCATE(RH,SH,UXXI,UYXI,UZXI,VXXI,VYXI,VZXI,WXXI,WYXI,WZXI, &
                 & UXET,UYET,UZET,VXET,VYET,VZET,WXET,WYET,WZET, &
                 & UXZE,UYZE,UZZE,VXZE,VYZE,VZZE,WXZE,WYZE,WZZE)
  ! 処理終了 ***********************************************************
  RETURN
! 内部手続き ***********************************************************
CONTAINS
