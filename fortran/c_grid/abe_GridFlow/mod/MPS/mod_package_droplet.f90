...
&            SA )
 ...
 enddo
 do k = ks + 1, ke - 1
  SA(is,k) = 0.5 * SA(is+1,k)
  SA(ie,k) = 0.5 * SA(ie-1,k)
 enddo
 do i = is + 1, ie - 1
  SA(i,ks) = 0.5 * SA(i,ks+1)
  SA(i,ke) = 0.5 * SA(i,ke-1)
 enddo
 SA(is,ks) = 0.25 * ( SA(is+1,ks) + SA(is,ks+1) )
 SA(ie,ks) = 0.25 * ( SA(ie-1,ks) + SA(ie,ks+1) )
 SA(is,ke) = 0.25 * ( SA(is+1,ke) + SA(is,ke-1) )
 SA(ie,ke) = 0.25 * ( SA(ie-1,ke) + SA(ie,ke-1) )
 ! メモリ解放 ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
 deallocate(s1, s2, s3)
 ! 処理終了 ********************************************************************************************
 return
...
&            is, ie, ks, ke, nDrp, MVD, LWC, Rhod, Ain, Uin, Nimp, SA, &
&            bet )
 ! 変数宣言 ********************************************************************************************
 implicit none
 ! 引数変数 ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
