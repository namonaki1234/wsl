...
def collect_battle_hrefs_from_page(page, page_no: int) -> list[str]:
    """
    指定ページから battle href を全部集める。
    """
    hrefs = []
    search_url = SEARCH_URL.format(query=quote(DEFAULT_USER_ID, safe=""), page=page_no)

    log(f"DEBUG: page={page_no} を取得中: {search_url}")
    ...
    log(f"DEBUG: page={page_no} の詳細リンク数 = {count}")
...
def scrape_all_target_pages(user_id: str, target_pages: list[int], existing_ids: set[str]):
    """
    page=1,2,3... を順に見て、まだDBに無い局だけ KIF を取得する。
    """
    new_games = []
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            for page_no in target_pages:
                ...
                log(f"DEBUG: 検索ページ取得: {search_url}")
                page.goto(search_url, wait_until="domcontentloaded")
                ...
                for i in range(link_count):
                    href = detail_links.nth(i).get_attribute("href")
                    if href and href != "/" and href not in seen_hrefs:
                        seen_hrefs.add(href)
                        hrefs.append(href)
                ...
                for idx, href in enumerate(hrefs, start=1):
                    ...
                    if game_id in existing_ids:
                        log(f"SKIP: 既存対局なので保存しない -> {game_id}")
                        continue

                    log(f"DEBUG: 新規対局を取得中 page={page_no} {idx}/{len(hrefs)} -> {game_id}")
...
# 全ページ解析
# =========================
def analyze_and_save_all_target_pages(user_id: str) -> None:
    """
    指定ページ群を全部見て、未保存の局だけ追加する。
    """
    log("analyze_and_save_all_target_pages 開始")

    existing_ids = get_existing_game_ids()
    log(f"DEBUG: 既存対局数 = {len(existing_ids)}")

    new_games = scrape_all_target_pages(user_id, TARGET_PAGES, existing_ids)
    ...
    log("analyze_and_save_all_target_pages 完了")


if __name__ == "__main__":
    init_db()
    analyze_and_save_all_target_pages(DEFAULT_USER_ID)