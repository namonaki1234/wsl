...
module Package_Icing
 ...
 public :: WallFrictionVelocityLRe2D
...
!******** 壁面摩擦速度 (二次元)									********
!*******************************************************************************************************
subroutine WallFrictionVelocityLRe2D( &
 &           is, ie, yp, up, nup, utau )
 ! 変数宣言 ********************************************************************************************
 implicit none
 ! 引数変数 ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
 integer, intent(in)  :: is, ie
 ...
 utau(:) = 0.0
 do i = is + 1, ie - 1
  !低Re数型モデルの場合,第一格子点は粘性底層(u+=y+)
  utau(i) = sqrt(nup(i) * up(i) / yp(i))
 enddo
 ! 処理終了 ********************************************************************************************
 return
end subroutine WallFrictionVelocityLRe2D
!*******************************************************************************************************
!******** 壁面摩擦速度 (三次元)									********
!*******************************************************************************************************
...
&            is, ie, Ue, mu, rho, utau, RH, hc )
 ! 変数宣言 ********************************************************************************************
 implicit none
 ! 引数変数 ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
 integer, intent(in)  :: is, ie
 real   , intent(in)  :: Ue(is:ie)					! 境界層外端の速度
 real   , intent(in)  :: mu(is:ie)
 real   , intent(in)  :: rho(is:ie)
...
&            is, ie, Ts, LWC, RH )
 ...
 ! 局所定数 ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
 real   , parameter :: k0 = 0.628 * 1.0e-3				! 物体の粗さ
 ! 局所変数 ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
 integer :: i
 ! 処理開始 ********************************************************************************************
 RH(:) = 0.0
 do i = is + 1, ie - 1
  RH(i) = k0 * 0.6839 * (0.047 * Ts(i) - 11.27) * (0.571 + 0.246 * LWC + 1.257 * LWC**2)
  if( RH(i) <= 0.0 ) RH(i) = k0
 enddo
 ! 処理終了 ********************************************************************************************
 return
...
!!*******************************************************************************************************
!subroutine EvapolationExt2D( &
!&            is, ie, Ta, Ti, Tw, Pt, hc, nPhase, Mes )
! ! 変数宣言 ********************************************************************************************
! implicit none
! ! 引数変数 ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
! integer, intent(in)  :: is, ie
! real   , intent(in)  :: Ta(is:ie), Ti(is:ie), Tw(is:ie), Pt(is:ie)
! real   , intent(in)  :: hc(is:ie)
! integer, intent(in)  :: nPhase(is:ie)
! real   , intent(out) :: Mes(is:ie)
! ! 局所変数 ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
!!    Tsur = Ti(i)
!!    kai  = 0.622 * hc(i) * LS / ( Cpa * Pt(i) * (1.0 / Pr)**(2.0 / 3.0) )
!!    Qes  = kai * e0 * (Tsur - Ta(i))
!!    Mes(i) = Qes / LS
!!    if( Mes(i) < 0.0 ) Mes(i) = 0.0
!!! 雨氷 -------------------------------------------------------------------------------------------------
!!! 蒸発
!!   case(2)
!!    Tsur = Tw(i)
...
&            is, ie, Tfree, Ta, Pt, Ufree, hc, dBi, Mim, Mrin, Mst, Mes, nPhase, Q0, Q1 )
 ! 変数宣言 ********************************************************************************************
 implicit none
 ! 引数変数 ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
 integer, intent(in)  :: is, ie
 real   , intent(in)  :: Tfree, Ufree
 real   , intent(in)  :: Ta(is:ie), Pt(is:ie)
 real   , intent(in)  :: hc(is:ie), dBi(is:ie)
 real   , intent(in)  :: Mim(is:ie), Mrin(is:ie), Mst(is:ie), Mes(is:ie)
 integer, intent(in)  :: nPhase(is:ie)
 real   , intent(inout) :: Q0(is:ie), Q1(is:ie)
 ! 局所変数 ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
 integer :: i
 real    :: kai
 ! 処理開始 ********************************************************************************************
...
!    Q0(i) = Rhor * LF * dBi(i) + Q0(i)
    Ti(i) = (Q0(i) - Q1(i) * Ts(i)) / (ki + Q1(i) * Bi(i)) * Bi(i) + Ts(i)
    if(Ti(i) > Ts(i)) Ti(i) = Ts(i)
    ! 水膜厚さ
    Bw(i) = 0.0
   ! 雨氷 ----------------------------------------------------------------------------------------------
   case(2)
...
!    &     - rhog / rhow * (Bi(i) - Bg(i))					! 式(32)
    ...
    ! 水膜温度
    Tw(i) = (Q0(i) - Q1(i) * Tf) / (kw + Q1(i) * Bw(i)) * Bw(i) + Tf
    if(Tw(i) > Tf) Tw(i) = Tf
    ! 氷層厚さ
    if( Bi(i) > 0.0 ) then
...
&            is, ie, hc, Tfree, Ts, Ta, Pt, Ufree, Mim, Mrin, Mst, Mes, time, dt, Bi, Q0, Q1, Bg, tg, nPhase )
 ! 変数宣言 ********************************************************************************************
 implicit none
 ! 引数変数 ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
 integer, intent(in)  :: is, ie
...
&            is, ie, dt, alp, dBi, Bi0, Bi )
 ...
 do i = is + 1, ie - 1
  if(alp == 0.0) then
    ...
    dBiAve  = 0.5 * (dBi(i-1) + dBi(i+1))
    Bi(i) = Bi0(i) + dt * min(dBi(i), dBiAve )
!    Bi(i) = Bi0(i) + dt * min(dBi(i), dBiAve * alp)
  endif
 enddo
 ! 処理終了 ********************************************************************************************
 return
