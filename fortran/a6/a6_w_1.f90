...
double precision,dimension(0:IM,0:JM) :: xix,xi_y,eta_x,eta_y,Jac
...
subroutine tvd_xi
  do j = 1,JM-1
    ...
    do i = 0,IM
      DUM = dSQRT(G*R*T(I,J)*(xix(I,J)**2+xi_y(I,J)**2))
      EIG(1,I) = xix(I,J)*U(I,J)+xi_y(I,J)*V(I,J)
      EIG(2,I) = EIG(1,I)
      EIG(3,I) = EIG(1,I)+DUM
      EIG(4,I) = EIG(1,I)-DUM
    end do
    ...
    do I = 0,IM
      UCONT = xix(I,J)*U(I,J)+xi_y(I,J)*V(I,J) !(反変速度)
      E(1,I) = Q(1,I,J)*UCONT
...
subroutine viscosity_calc
  ...
  do j = 0,JM
    do i = 0,IM
      txx = 2.0d0/3.0d0*mu*(2.0d0*(u_xi(i,j)*xix(i,j)+u_eta(i,j)*eta_x(i,j))-(v_xi(i,j)*xi_y(i,j)+v_eta(i,j)*eta_y(i,j)))
      tyy = 2.0d0/3.0d0*mu*(2.0d0*(v_xi(i,j)*xi_y(i,j)+v_eta(i,j)*eta_y(i,j))-(u_xi(i,j)*xix(i,j)+u_eta(i,j)*eta_x(i,j)))
      txy = mu*((u_xi(i,j)*xi_y(i,j)+u_eta(i,j)*eta_y(i,j))+(v_xi(i,j)*xix(i,j)+v_eta(i,j)*eta_x(i,j)))
      tyx = txy
      R4 = txx*u(i,j)+txy*v(i,j)+mu/(Pr*(g-1.0d0))*g*r*(T_xi(i,j)*xix(i,j)+T_eta(i,j)*eta_x(i,j))
      S4 = tyx*u(i,j)+tyy*v(i,j)+mu/(Pr*(g-1.0d0))*g*r*(T_xi(i,j)*xi_y(i,j)+T_eta(i,j)*eta_y(i,j))
      RRR(1,i,j) = 0.0d0
      RRR(2,i,j) = (xix(i,j)*txx+xi_y(i,j)*txy)/Jac(i,j)
      RRR(3,i,j) = (xix(i,j)*tyx+xi_y(i,j)*tyy)/Jac(i,j)
      RRR(4,i,j) = (xix(i,j)*R4+xi_y(i,j)*S4)/Jac(i,j)
      SSS(1,i,j) = 0.0d0
