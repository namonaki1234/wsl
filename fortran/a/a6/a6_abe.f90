...
double precision,parameter :: H = 1.0d-2 !基準長さ（高さ）
integer,parameter :: IM = 30 !x方向領域数
integer,parameter :: JM = 30 !y方向領域数
double precision,parameter :: dt = 1.0d-8 !時刻刻み幅
  ...
  do nn = 1,4
    ...
    do j = 0,JM
      do i = 0,IM
        ...
        if (i==0) then
          uxi(i,j) = (-3.0d0*u(i,j)+4.0d0*u(i+1,j)-u(i+2,j))/2.0d0
          vxi(i,j) = (-3.0d0*v(i,j)+4.0d0*v(i+1,j)-v(i+2,j))/2.0d0
          Txi(i,j) = (-3.0d0*T(i,j)+4.0d0*T(i+1,j)-T(i+2,j))/2.0d0
        end if
        if (i==IM) then
          uxi(i,j) = (3.0d0*u(i,j)-4.0d0*u(i-1,j)+u(i-2,j))/2.0d0
          vxi(i,j) = (3.0d0*v(i,j)-4.0d0*v(i-1,j)+v(i-2,j))/2.0d0
        ...
        if (j==0) then
          ueta(i,j) = (-3.0d0*u(i,j)+4.0d0*u(i,j+1)-u(i,j+2))/2.0d0
          veta(i,j) = (-3.0d0*v(i,j)+4.0d0*v(i,j+1)-v(i,j+2))/2.0d0
          Teta(i,j) = (-3.0d0*T(i,j)+4.0d0*T(i,j+1)-T(i,j+2))/2.0d0
        end if
        if (j==JM) then
          ueta(i,j) = (3.0d0*u(i,j)-4.0d0*u(i,j-1)+u(i,j-2))/2.0d0
          veta(i,j) = (3.0d0*v(i,j)-4.0d0*v(i,j-1)+v(i,j-2))/2.0d0
    ...
    do i = 0,IM !粘性項ベクトル
      do j = 0,JM
        txx = 2.0d0*mu(i,j)*(2.0d0*(uxi(i,j)*xix(i,j)+ueta(i,j)*etax(i,j))-(vxi(i,j)*xiy(i,j)+veta(i,j)*etay(i,j)))/3.0d0
        txy = mu(i,j)*((uxi(i,j)*xiy(i,j)+ueta(i,j)*etay(i,j))+(vxi(i,j)*xix(i,j)+veta(i,j)*etax(i,j)))
        tyy = 2.0d0*mu(i,j)*(2.0d0*(vxi(i,j)*xiy(i,j)+veta(i,j)*etay(i,j))-(uxi(i,j)*xix(i,j)+ueta(i,j)*etax(i,j)))/3.0d0
        Rv(2,i,j) = txx
        Rv(3,i,j) = txy
        Rv(4,i,j) = txx*u(i,j)+txy*v(i,j)+mu(i,j)*g*R*(Txi(i,j)*xix(i,j)+Teta(i,j)*etax(i,j))/(Pr*(g-1.0d0))
        Sv(2,i,j) = txy
        Sv(3,i,j) = tyy
        Sv(4,i,j) = txy*u(i,j)+tyy*v(i,j)+mu(i,j)*g*R*(Txi(i,j)*xiy(i,j)+Teta(i,j)*etay(i,j))/(Pr*(g-1.0d0))
        do k = 1,4 !各流束を一般座標系に変換
          Rvh = Rv(k,i,j)
          Rv(k,i,j) = (xix(i,j)*Rv(k,i,j)+xiy(i,j)*Sv(k,i,j))/JJ(i,j)
          Sv(k,i,j) = (etax(i,j)*Rvh+etay(i,j)*Sv(k,i,j))/JJ(i,j)
        end do
      end do
    end do
    ...
    do j = 1,JM-1
      do i = 1,IM-1
        do k = 1,4
          QQ(k,i,j,nn) = QQ(k,i,j,0)-dt*(EE(k,i,j)-EE(k,i-1,j)+FF(k,i,j)-FF(k,i,j-1) &
                                         -(Rv(k,i+1,j)-Rv(k,i-1,j)+Sv(k,i,j+1)-Sv(k,i,j-1))/2.0d0)/dble(5-nn)
          Q(k,i,j) = QQ(k,i,j,nn)
        end do
      end do
    end do
...
!.datファイル書き出し***************************************
open (1,file='comp6.dat',status='replace')
do j = 0,JM
  do i = 0,IM
...
!.fldファイル作成*******************************************
open (2,file='comp6.fld',status='replace')
...
write (2,'(A)') 'variable 1 file=comp6.dat filetype=ascii skip=0 offset=2 stride=6'
write (2,'(A)') 'variable 2 file=comp6.dat filetype=ascii skip=0 offset=3 stride=6'
write (2,'(A)') 'variable 3 file=comp6.dat filetype=ascii skip=0 offset=4 stride=6'
write (2,'(A)') 'variable 4 file=comp6.dat filetype=ascii skip=0 offset=5 stride=6'
write (2,'(A)') 'coord 1 file=comp6.dat filetype=ascii skip=0 offset=0 stride=6'
write (2,'(A)') 'coord 2 file=comp6.dat filetype=ascii skip=0 offset=1 stride=6'
close (2)
